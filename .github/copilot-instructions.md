...
This document provides comprehensive guidelines for creating syntactically correct Dynatrace Workflows. These instructions are designed for AI coding assistants including GitHub Copilot, Cursor, and Kiro.
...
metadata:
  ...
  dependencies:
    apps:
      - id: dynatrace.automations  # Required for all workflows
        version: ^1.840.0
...
- `dynatrace.automations` - Core workflow actions (required)
...
#### 5. Looping with with-items (For-Each)
...
send_notifications:
  ...
  with-items: "owner in {{ result('get_owners').slackChannels }}"
...
- **Workflow Samples**: This repository contains many examples in `/samples`
